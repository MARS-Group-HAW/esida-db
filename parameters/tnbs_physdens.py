...
class tnbs_physdens(BaseParameter):
    ...
    def load(self, shapes=None, save_output=False):
        ...
        for name, shape_id in name_to_id.items():
            ...
            dfx = df[df['HEALTH WORKERS'].isin([
                'DENSITY OF MEDICAL OFFICERS (MOS) AND ASSISTANT MEDICAL OFFICERS (AMOS) PER 10000 POPULATION', # id 7
                'DENSITY OF CLINICIANS PER 10000 POPULATION', # id 10
            ])]

            rows.append({
                'year': 2014,
                'shape_id': shape_id,
                f"{self.parameter_id}": dfx[df_name].sum()
            })
